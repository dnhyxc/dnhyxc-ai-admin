...
| [ui](./ui/) | UI 组件库（基于 Radix UI 的基础组件封装、全局表格样式覆盖） | [UI 组件库实现](./ui/ui-components.md)、[表格顶部圆角去除](./ui/表格顶部圆角去除.md) |
| [layout](./layout/) | 布局与导航（AdminLayout、菜单、路由、权限过滤、侧栏命名风格） | [布局与导航实现](./layout/admin-layout.md)、[角色菜单与权限过滤](./layout/角色菜单与权限过滤.md)、[菜单项标签重命名](./layout/菜单项标签重命名.md) |
